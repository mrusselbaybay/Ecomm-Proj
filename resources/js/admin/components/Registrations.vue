...
<template>
  <div>
    ...
    <div class="flex gap-4 mb-4 flex-wrap">
      <input type="text" v-model="search" placeholder="Search applicants..." class="field-input w-64" @input="loadData" />
      ...
      <table class="admin-table">
        ...
        <tbody>
          ...
          <tr v-else-if="registrations.length === 0">
            <td colspan="6" class="text-center py-4 text-slate-500">No pending registrations found.</td>
          </tr>
          <tr v-for="(user, idx) in registrations" :key="idx">
            ...
            <td>
              <div class="flex gap-2">
                ...
                <button @click="openRejectModal(user)" class="btn-danger-outline" :disabled="user.status === 'rejected'">Reject</button>
...
function viewRegistration(user) {
  alert(`📄 Viewing documents for ${user.full_name || user.first_name || user.email}`);
}
