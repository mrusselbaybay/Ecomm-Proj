<!-- resources/js/logistics/components/DeliveryAreas.vue
     Delivery-area coverage + the area editor (location rules and the
     per-area assigned-drivers tab). Split out of the old combined
     "Riders & Areas" page so this and the rider roster (Riders.vue) load
     independently — opening one no longer pulls in the other's UI. -->
<template>
                    ...
                    >
                        ...
                        <template v-if="activeTab === 'location'">
                            <div class="area-modal-body">
                                <p class="modal-desc">
                                    Use the parcel's province and municipality
                                    to determine its destination area.
                                </p>
                                <div class="area-form-grid">
                                    ...
                                    <label class="form-field full-field"
                                        ...
                                        <div
                                            v-if="form.municipalities.length"
                                            class="municipality-chip-list"
                                        >
                                            <span
                                                v-for="m in form.municipalities"
                                                :key="m.code || m.name"
                                                class="badge badge-indigo municipality-chip"
                                            >
                                                {{ m.name }}
                                                <button
                                                    type="button"
                                                    class="municipality-chip-remove"
                                        ...
                                        <small v-else class="field-hint"
                                            >Add at least one municipality or
                                            city.</small
                                        ></label
                                    >
                                    <p
                        ...
                        <template v-else>
                            <div class="area-modal-body">
                                ...
                                <div class="driver-list">
                                    ...
                                    >
                                        No drivers assigned yet. Click "Add
                                        courier" to appoint one.
                                    </p>
                                    <div
...
const form = reactive({
    ...
    province_code: '',
    province_name: '',
    // One province (above), any number of municipalities/cities within
    // it — replaces the old single municipality_code/municipality_name
    // pair. Each entry is { code, name, barangay }; barangay stays null
    // (covers the whole municipality) unless narrowed later.
    municipalities: [],
    is_active: true,
});
// The municipality picker that feeds "Add" below — separate from the
// list itself so picking one doesn't require re-selecting from scratch.
const pendingMunicipalityCode = ref('');
...
const editingArea = computed(
    () =>
        deliveryAreas.value.find((area) => area.id === editingAreaId.value) ||
        null,
);
...
// sessionStorage and collapses concurrent callers onto one request. Both
// this page and Account Settings read through it, so the province list is
// fetched at most once per session no matter how often either form is
// opened. Only province + municipality are needed here — no barangay step.
...
function onProvinceChange() {
    ...
    // Changing province invalidates whatever municipalities were picked
    // for the old one — they're not valid coverage for the new province.
    form.municipalities = [];
    pendingMunicipalityCode.value = '';
    fetchMunicipalities(form.province_code);
}

function addMunicipality() {
    const selected = municipalityOptions.value.find(
        (m) => m.code === pendingMunicipalityCode.value,
    );
    ...
    if (!form.municipalities.some((m) => m.code === selected.code)) {
        form.municipalities.push({
            code: selected.code,
            name: selected.name,
            barangay: null,
        });
...
function removeMunicipality(code) {
    form.municipalities = form.municipalities.filter((m) => m.code !== code);
}
...
async function openAreaModal(area = null) {
    ...
    const matchedProvince = provinceOptions.value.find(
        (p) =>
            p.name.toLowerCase() === (area.province_name || '').toLowerCase(),
    );
    ...
    if (!matchedProvince) {
        ...
        form.municipalities = (area.municipalities || []).map((m) => ({
            code: m.code || null,
            name: m.name,
    ...
    // Match each saved municipality by name against the freshly fetched
    // list to fill in a code where one wasn't stored yet.
    form.municipalities = (area.municipalities || []).map((saved) => {
        const matched = municipalityOptions.value.find(
            (m) => m.name.toLowerCase() === (saved.name || '').toLowerCase(),
        );

        return {
            code: matched?.code || saved.code || null,
            name: matched?.name || saved.name,
            barangay: saved.barangay || null,
        };
    });
}
...
async function submitArea() {
    if (form.municipalities.length === 0) {
        formError.value = 'Add at least one municipality or city.';
        ...
        const saved = await saveDeliveryArea(
            {
                ...
                municipalities: form.municipalities.map((m) => ({
                    code: m.code || undefined,
                    name: m.name,
                    barangay: m.barangay || undefined,
                })),
...
async function removeArea() {
    ...
    // editingAreaId, which would otherwise send DELETE .../areas/null.
    const areaId = editingAreaId.value;
    ...
    const ok = await askConfirm({
        ...
        message: `Delete ${form.name}? This routing rule cannot be recovered, and any parcels routed to it will need re-sorting.`,
...
async function load(force = false) {
    ...
    try {
        await loadDeliveryAreas({ force });
    } catch (error) {
        notifyError(error, 'Could not load delivery areas.');
    } finally {
        loading.value = false;
...
onMounted(() => {
    ...
    // delivery area" modal opens instantly when it's needed. usePsgc
