...
const verifiedDocsCount = computed(() => documents.value.filter(d => d.status === 'approved').length);
const pendingDocsCount = computed(() => documents.value.filter(d => d.status === 'pending').length);
...
async function loadProfileData() {
  ...
  const [{ data: addr }, { data: details }, { data: docs }] = await Promise.all([
    supabase.from('addresses').select('*').eq('profile_id', uid).eq('owner_kind', 'profile').maybeSingle(),
    supabase.from('seller_details').select('*').eq('profile_id', uid).maybeSingle(),
    supabase.from('documents').select('*').eq('profile_id', uid).order('created_at', { ascending: false }),
  ]);
