...
const { getOrderById, statusBadgeClass, formatCurrency, cancelOrder } =
    useOrders();
...
function trackPackage() {
    ...
    if (order.value?.shipping?.trackingNumber) {
        window.alert(
            `Tracking number: ${order.value.shipping.trackingNumber}\n(A full tracking view isn't wired up yet.)`,
        );
    }
}
...
async function handleCancel() {
    ...
    if (
        !window.confirm(
            `Cancel order ${order.value.id}? This cannot be undone.`,
        )
    ) {
        return;
    }
