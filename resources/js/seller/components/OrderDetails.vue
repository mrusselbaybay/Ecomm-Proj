        ...
        <template v-else>
            ...
            <div class="order-detail-page-grid">
                ...
                <div class="order-detail-page-right">
                    ...
                    <!-- Parcel confirmation QR — present once dispatch-prep has
                         minted one (see Prepare Orders / SellerOrderController::
                         dispatchPrep). The courier scans this at pickup and
                         again on delivery. -->
                    <div
                        v-if="order.dispatch && order.dispatch.qrPayload"
                        class="card od-card order-parcel-qr-card"
                    >
                        <h3 class="order-section-label">
...
async function loadOrder() {
    ...
    const { order: fetched, error } = await getOrderById(props.orderId, { includeJourney });
