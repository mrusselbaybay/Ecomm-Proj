<!-- resources/js/buyer/components/AvatarCropper.vue
     Shown between "pick a file" and "actually upload it" for every avatar
     upload in the app (buyer/seller/admin each carry their own copy of
     this component, matching how this codebase already duplicates small
     per-module pieces — e.g. the PSGC address-lookup functions — rather
     than sharing a file across separate Vite entry bundles).
...
<template>
    <div v-if="file" class="cropper-overlay" @click.self="$emit('cancel')">
        <div class="cropper-panel">
            ...
            <p class="cropper-hint">Drag to reposition, use the slider to zoom.</p>
            ...
            <div class="cropper-actions">
                <button type="button" class="cropper-btn-outline" @click="$emit('cancel')">Cancel</button>
                <button type="button" class="cropper-btn-primary" :disabled="!ready" @click="saveCrop">Save Photo</button>
            </div>
        </div>
    </div>
</template>
...
const dispWidth = computed(() => naturalWidth.value * baseScale.value * zoom.value);
const dispHeight = computed(() => naturalHeight.value * baseScale.value * zoom.value);
...
function onImageLoad(event) {
    ...
    baseScale.value = Math.max(VIEWPORT / img.naturalWidth, VIEWPORT / img.naturalHeight);
    zoom.value = 1;
    offsetX.value = 0;
    offsetY.value = 0;
    ready.value = true;
...
function saveCrop() {
    ...
    img.onload = () => {
        ...
        const sourceX = (dispWidth.value / 2 - VIEWPORT / 2 - offsetX.value) / scaleFactor;
        const sourceY = (dispHeight.value / 2 - VIEWPORT / 2 - offsetY.value) / scaleFactor;
