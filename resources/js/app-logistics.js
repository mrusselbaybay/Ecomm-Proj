...
// ---------- Portal ----------
// This is the dedicated logistics auth page — a copy of the buyer/seller
// auth page (resources/js/app.js) restricted to logistics accounts only.
// A login for any other role is blocked here with a redirect prompt back
// to the main buyer/seller auth page instead of being let in.
...
const App = {
    setup() {
        ...
        // Set when a login here belongs to the buyer/seller portal
        // instead of logistics — shows a warning modal with a link to
        ...
        // Role Options — logistics portal only offers the logistics role.
        const roles = [
            {
                id: 'logistics',
                label: 'Logistics',
        ...
        function switchMode(next) {
            ...
            // Logistics is the only role on this portal — skip the picker.
            if (next === 'signup') {
        ...
        onMounted(async () => {
            ...
            } else if (/^\/(logistics-)?signup/.test(window.location.pathname)) {
  ...
  <div class="min-h-screen flex flex-col md:flex-row" style="height:100vh;overflow:hidden;">
    ...
    <div v-if="portalMismatch" class="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/60 px-4">
      <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
        ...
        <p class="text-slate-800 font-semibold mb-1">This looks like a buyer or seller account.</p>
        <p class="text-slate-500 text-sm mb-6">Logistics accounts sign in here. Please proceed to the main BuyTheWay login.</p>
        <a href="/login" class="btn-gradient block w-full text-white font-semibold py-2.5 rounded-lg mb-2">Go to BuyTheWay Login →</a>
