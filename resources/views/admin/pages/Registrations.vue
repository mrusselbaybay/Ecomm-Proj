...
<template>
  <div>
    ...
    <div class="flex gap-4 mb-4 flex-wrap">
      <input type="text" v-model="search" placeholder="Search applicants..." class="field-input w-64" />
    ...
    <div class="card overflow-hidden">
      <table class="admin-table">
        <thead>
          <tr><th>Applicant</th><th>Role</th><th>Email</th><th>Submitted</th><th>Status</th><th>Actions</th></tr>
        </thead>
        <tbody>
          <tr v-if="loading">
          ...
          <tr v-else-if="registrations.length === 0">
            <td colspan="6" class="text-center py-4 text-slate-500">No registrations found.</td>
          </tr>
          <tr v-for="(user, idx) in registrations" :key="idx">
            ...
            <td>
              ...
              <span class="badge" :class="statusBadgeClass(user.status)">{{ user.status }}</span>
            </td>
            <td>
              <div class="flex gap-2">
                <button @click="approveUser(user)" class="btn-sm-gradient" :disabled="user.status === 'approved'">Approve</button>
                <button @click="rejectUser(user)" class="btn-danger-outline" :disabled="user.status === 'rejected'">Reject</button>
                <button @click="viewRegistration(user)" class="btn-outline">View Docs</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="showRejectModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="card p-6 w-96">
        ...
        <div class="flex gap-2 mt-4">
          <button @click="showRejectModal = false" class="btn-outline flex-1 py-2">Cancel</button>
          <button @click="submitRejection" class="btn-danger-outline flex-1 py-2">Reject</button>
        </div>
      </div>
    </div>
  </div>
