...
it('shows seller attachments in the buyer conversation', function () {
    ...
    // Conversation detail no longer inlines messages (they're paginated
    // separately so opening a thread doesn't pull its whole history) — see
    // Buyer\MessageController::showConversation()/messages().
    $this->getJson("/api/buyer/messages/conversations/{$conversation->id}/messages")
        ->assertOk()
        ->assertJsonPath('data.0.attachments.0.name', 'product-photo.jpg')
        ->assertJsonPath('data.0.attachments.0.mime', 'image/jpeg')
        ->assertJsonPath('data.0.attachments.0.url', 'data:image/jpeg;base64,dGVzdA==');
});
