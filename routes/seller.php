 ...
 * never trusted from the client either). Product routes: category/status/
 * price/stock/variant data must be validated and enforced server-side
 * (see SellerProductService), which isn't possible if the SPA writes
 * directly to Supabase, so products moved onto this same pattern as
 * orders below.
 */
Route::middleware(['supabase.auth', 'seller'])->prefix('api/seller')->name('api.seller.')->group(function () {
    ...
    // feeds the Courier/Carrier dropdown on Prepare Shipment and Courier
    // Handover (see SellerCourierController).
