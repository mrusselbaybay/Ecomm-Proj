...
class SellerProductService
{
    ...
    public function create(Profile $seller, array $data): Product
    {
        return DB::transaction(function () use ($seller, $data) {
            ...
            // AI moderation gate — see ModerateProductJob/ProductApprovalRouter.
            // Deferred via afterCommit() so it never fires on a rolled-back
            // create (e.g. a variant/option validation failure above).
    ...
    public function update(Profile $seller, Product $product, array $data): Product
    {
        return DB::transaction(function () use ($seller, $product, $data) {
            ...
            // Editing resets status to pending_review above, so re-run the
            // same AI moderation gate an edited listing goes through on
            // create — never trust a previously-approved product to still
            // be compliant after its content changes.
            ModerateProductJob::dispatch($product->id)->afterCommit();
