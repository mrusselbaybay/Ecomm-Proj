...
/**
 * The company's province-wide fallback rider pool — every barangay in its
 * own province that isn't individually covered falls back here. Car or van
 * riders only (see App\Support\VehicleCategory). At most one per company,
 * auto-provisioned off its own address (App\Services\ServiceAreaProvisioner)
 * rather than created by hand here.
 */
class LogisticsProvincialAssignmentController extends Controller
{
    use ChecksRiderCoverage;

    /**
     * The company's provincial pool and its current riders. Provisions it
     * on first visit if the company has an address on file but hasn't
     * saved a barangay assignment yet.
     */
    public function show(Request $request): JsonResponse
    {
        ...
        if (! $assignment) {
            return response()->json([
                'assignment' => null,
                'message' => 'Add a business address in Account Settings first — the provincial pool is scoped to your own province.',
            ]);
        }
     ...
     * area, and driving a car, van, or truck.
     */
    ...
    /**
     * Add one or more riders to the pool in one request — the picker lets
     * staff multi-select before confirming.
     */
    ...
    private function assignmentFor(LogisticsCompany $company): ?LogisticsProvincialAssignment
    {
        $provisioned = app(ServiceAreaProvisioner::class)->ensureForCompany($company);

        return $provisioned['provincial'];
    }
    ...
    private function eligibleRiderQuery(LogisticsCompany $company, string $search): Builder
    {
        return Profile::query()
            ...
            ->whereIn(DB::raw('LOWER(courier_details.vehicle::text)'), ['car', 'van', 'truck'])
    ...
    private function ensureVehicleEligible(string $riderProfileId): void
    {
        ...
        if (! $rider || ! VehicleCategory::satisfiesCar($rider->vehicleLabel())) {
            throw ValidationException::withMessages([
                'rider_profile_ids' => 'Only riders with a car, van, or truck can join the provincial pool.',
            ]);
        }
    }
    ...
    private function transform(LogisticsProvincialAssignment $assignment): array
    {
        return [
            'id' => $assignment->id,
            'province_name' => $assignment->province_name,
